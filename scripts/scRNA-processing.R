...
if("full" %in% pipeline.to.run){
  pipeline.to.run <- c("filter", "merge", "processing", "integration", "annotation")
}

# Function to run filtering step
run_filter <- function(in.path, out.path, params) {
  ...
  seurat.list <- lapply(seq_along(sample.dirs), function(i) {
    dir <- sample.dirs[i]
    dir <- file.path(dir, "data/processed")
    sample <- samples.id[i]
    # fn <- paste0(params$project.prefix, "_", sample, "_minimal.rds")
    fn <- paste0(params$project.prefix, "_", sample, "_final.rds")
  ...
  saveRDS(seurat.list, file = file.path(out.path, 
...
run_processing <- function(seu, params, out.path) {
...
run_integration <- function(seu, params, out.path) {
  ...
  if(params$normalization == "SCT") {
    ...
    seu <- FindNeighbors(seu, reduction = "integrated", dims = 1:params$n.dim.integration) %>% 
           FindClusters(resolution = params$cluster_resolutions) %>%
...
run_annotation <- function(seu, params, out.path) {
  ...
  dir.create(file.path(out.path, "plots"), recursive = T)
  pdf(file.path(out.path, "plots", paste0(params$project.prefix, "-annotated-obj.pdf")))
...
main <- function() {
  ...
  in.path <- params$output
  out.path <- file.path(in.path, "output-combined")
  ...
  if("filter" %in% pipeline.to.run) {
    ...
    seurat.list <- run_filter(in.path, out.path, params)
  }
  
  if("merge" %in% pipeline.to.run && !is.null(seurat.list)) {
    message("\n\n++++++++ Merging ++++++++++")
    seu <- run_merge(seurat.list, out.path)
  }
  
  if("processing" %in% pipeline.to.run && !is.null(seu)) {
    message("\n\n++++++++ Processing ++++++++++")
    seu <- run_processing(seu, params, out.path)
  }
  
  if("integration" %in% pipeline.to.run && !is.null(seu)) {
    message("\n\n++++++++ Integration ++++++++++")
    seu <- run_integration(seu, params, out.path)
  ...
  if("annotation" %in% pipeline.to.run && !is.null(seu)) {
    message("\n\n++++++++ Annotation ++++++++++")
    seu <- run_annotation(seu, params, out.path)
  }
